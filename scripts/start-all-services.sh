...
# Script para iniciar todos os serviços do sistema de upload
...
echo ""
echo "📦 Iniciando Workers WebSocket..."

# Worker 1
echo -e "${GREEN}▶️  Iniciando Worker 1 (porta 20001)...${NC}"
php artisan websocket:start --port=20001 > storage/logs/websocket-worker-1.log 2>&1 &
WORKER1_PID=$!
echo "   PID: $WORKER1_PID"
# Aguarda worker iniciar antes do próximo
sleep 3
...
php artisan websocket:start --port=20010 > storage/logs/websocket-worker-2.log 2>&1 &
WORKER2_PID=$!
...
php artisan websocket:start --port=20020 > storage/logs/websocket-worker-3.log 2>&1 &
WORKER3_PID=$!
...
php artisan websocket:start --port=20040 > storage/logs/websocket-worker-4.log 2>&1 &
WORKER4_PID=$!
