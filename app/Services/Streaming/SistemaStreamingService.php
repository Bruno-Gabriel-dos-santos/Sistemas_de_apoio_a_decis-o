...
class SistemaStreamingService implements StreamingContextHandlerInterface
{
    public function resolveStoragePath(int $userId, string $relativePath, array $payload): string
    {
        $sistemaId = (int) ($payload['sistema_id'] ?? 0);

        if ($sistemaId <= 0) {
            throw new \RuntimeException('Sistema inválido para upload.');
        }
        ...
        return 'sistemas/' . $sistemaId . '/' . $safeRelative;
    }
    ...
    public function finalize(array $metadata, string $tempAbsolutePath, int $fileSize): array
    {
        ...
        if (!$destinationRelative) {
            throw new \RuntimeException('Destino não informado para upload em sistemas.');
        }
    ...
    protected function moveToFinalPath(string $tempAbsolutePath, string $destinationRelative): void
    {
        if (!file_exists($tempAbsolutePath)) {
            throw new \RuntimeException('Arquivo temporário não encontrado para upload em sistemas.');
        }
