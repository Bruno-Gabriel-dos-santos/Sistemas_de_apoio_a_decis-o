...
<div class="mx-auto py-8" style="width: 80%;">
    <div class="flex justify-end mb-4 gap-2">
        <a href="{{ route('relatorios-situacionais.edit', $post->id) }}" class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-4 rounded">Editar</a>
        <form id="form-deletar-{{ $post->id }}" action="{{ route('relatorios-situacionais.destroy', $post->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="button" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onclick="abrirModalSenha({{ $post->id }})">Deletar</button>
        </form>
    </div>
    <div class="bg-white rounded-lg shadow p-6">
        <img src="{{ asset('storage/' . $post->capa) }}" alt="Capa" class="w-full h-64 object-cover rounded mb-4">
        <h1 class="text-3xl font-bold mb-2">{{ $post->titulo }}</h1>
        <div class="flex justify-between text-sm text-gray-500 mb-4">
            <span>Autor: {{ $post->autor }}</span>
            <span>{{ \Carbon\Carbon::parse($post->data)->format('d/m/Y') }}</span>
            @if($post->tag)
                <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded">{{ $post->tag }}</span>
            @endif
        </div>
        <div class="prose max-w-none">{!! $post->conteudo !!}</div>
    </div>
</div>
<!-- Modal de senha -->
